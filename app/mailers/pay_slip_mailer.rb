class PaySlipMailer < ApplicationMailer
  ...
  def monthly(employee, pay_slip)
    ...
    employee_data = [
      ...
      ['Department', employee.department_name],
      ['Date of Joining', (employee.date_of_joining.strftime("%m/%d/%Y") rescue '')],
      ['Designation', employee.designation],
    ...
    pdf = Prawn::Document.new do
      ...
      move_down(50)
      text pdf_template
      table employee_data, width: bounds.width, header: true, row_colors: ['e6e6e6','ffffff'] do
        row(0).font_style = :bold
        column(0).background_color = 'e6e6e6'
      end
      move_down 20
      table salary_data, width: bounds.width, header: true, row_colors: ['e6e6e6','ffffff'] do
        row(0).font_style = :bold
        column(0).background_color = 'e6e6e6'
      end
